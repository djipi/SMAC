...
int smac_qsort(char *base, int n, int size, int	(*compar)()) {
...
char *amem(LONG amount) {
   ...
   if(amount < A_THRESH) {                                  // Honor *small* request
      if(a_amount < amount) {
         a_ptr = amem(A_AMOUNT);
         a_amount = A_AMOUNT;
      }
...
void display_version(void) {
   ...
   printf("Copyright (c) SubQMod 2007-2009\n"); 
   //printf("V%01i.%01i.%02i (Beta) %s (%s)\n\n", MAJOR, MINOR, PATCH, __DATE__, PLATFORM);
   printf("V%01i.%01i.%02i %s (%s)\n\n", MAJOR, MINOR, PATCH, __DATE__, PLATFORM);
}
...
void interactive(void) {
   ...
   // As there is no command line, print a copyright message and prompt for command line
	s = "*****************************************************\n";
   printf("\n%s* SMAC - SubQMod's Macro Assembler for Atari Jaguar *\n", s);
   printf("* Copyright (c) SubQMod, 2009                       *\n");
   printf("* Version %01i.%01i.%02i                Platform: %-9s *\n",MAJOR,MINOR,PATCH,PLATFORM);
   printf("* ------------------------------------------------- *\n");
...
int main(int argc, char **argv) {
   ...
   for(i = 0; i < MAXFWDJUMPS; i++ ) fwdjump[i] = 0;

   if(argc > 1) {                                           // Full command line passed
